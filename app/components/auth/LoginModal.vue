    ...
    <div class="auth-form">
      ...
      <div v-if="step === 1" class="auth-form-group">
        ...
        <div class="auth-form-link">
          ...
          <NuxtLink v-if="standalone" to="/register" class="auth-link">{{
            $t("auth.register")
          }}</NuxtLink>
          <button
            v-else
      ...
      <div v-else-if="step === 2" class="auth-form-group">
        ...
        <button type="button" class="auth-link-btn" @click="step = 1">
          ← {{ $t("auth.backToLogin") }}
        </button>
      </div>

      <div v-else class="auth-form-group auth-form-group-otp">
        <p class="auth-form-info">
          {{ $t("auth.otpSentTo") }} <b>{{ email }}</b
          >. {{ $t("auth.checkInbox") }}
        </p>
        ...
        <button type="button" class="auth-link-btn" @click="step = 2">
          ← {{ $t("auth.changeEmail") }}
        </button>
      </div>
...
const AuthForgotPasswordModal = defineAsyncComponent(
  () => import("~/components/auth/AuthForgotPasswordModal.vue"),
);
...
async function sendOtp() {
  ...
  error.value = "";
  try {
    await $fetch("/api/auth/send-otp-login", {
      method: "POST",
